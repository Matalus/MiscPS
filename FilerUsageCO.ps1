...
Write-Host ""
Write-Host "Transcript Started - Script Will Log output to \\vault\tech3\Scripts\Matt\logs\FilerUsageCO$d.log"
Write-Host ""

ForEach($Folder in Get-ChildItem \\vault\userdata$\~snapshot\daily.0\cousers | where { $_.name -notlike 'crossdep'} | where { $_.name -notlike 'shared'} | Where {$_.PSisContainer -eq $True} | Sort-Object){
