...
$CSV = Import-Csv \\vault\tech3\Scripts\Matt\movecsvday.csv
            ...
            Write-Host From: $Source
             Write-Host "To:  "  azmail\Storage Group $SG\Mailbox Database $SG
                ...
                IF($Source -eq "azmail\Storage Group $SG\Mailbox Database $SG")
                    ...
                    $REMAINING | Export-Csv \\vault\tech3\Scripts\Matt\movecsvday.csv -Force
                    Write-Host "CSV Updated..."
                    }
                    ELSE
                        {
                        Move-Mailbox -Identity $user -PreserveMailboxSizeLimit -BadItemLimit 50 -TargetDatabase "azmail\Storage Group $SG\Mailbox Database $SG" -Confirm:$False -MaxThreads 2 | Out-Null
