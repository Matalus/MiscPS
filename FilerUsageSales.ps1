...
Write-Host ""
Write-Host "Transcript Started - Script Will Log output to \\vault\tech3\Scripts\Matt\logs\FilerUsageSales$d.log"
Write-Host ""

ForEach($Folder in get-childitem \\vault\userdata$\~snapshot\daily.0\userssal | where { $_.name -notlike 'MyReports'} | where { $_.name -notlike 'profile'} | Where {$_.PSisContainer -eq $True} | Sort-Object){
