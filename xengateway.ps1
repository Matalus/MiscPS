...
foreach($ServerName in $Servers)
    {
    $ServerName
    $queryResults = Get-TSSession -ComputerName $ServerName | ?{$_.State -like "Listening" -and $_.WindowStationName -like "ica-tcp"}
    ...
    IF($queryResults -ne $null)
        ...
        ELSE
        {
        $RESET = Get-TSSession -ComputerName $ServerName | ?{$_.WindowStationName -like "ICA-tcp"} | Stop-TSSession -Confirm:$false
        Get-Service -ComputerName $ServerName -Name IMAService | Restart-Service -Force
        Start-Sleep -Seconds 30
        $Query2 = Get-TSSession -ComputerName $ServerName | ?{$_.State -like "Listening" -and $_.WindowStationName -like "ica-tcp"}
        IF($Query2 -ne $null)
            {
            ...
            $DOWN | Add-Member -NotePropertyName "Listener State" -NotePropertyValue "Down"
            [ARRAY]$RESULTS += $DOWN
            }
        } 
